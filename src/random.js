...
Flyer.random = new (class FlyerRandom {
    /**
     * returns a random number using the Math.random between [0, ceiling - 1]
    ...
    get(ceiling, useFloat) {
        ...
        let result;
        if (useFloat) {
            result = Math.random() * ceiling;
        } else {
            result = Math.floor(Math.random() * ceiling);
        }
        ...
        for (var i = 0; i < count; i++) {
            set.push(
                start +
                    i * interval +
                    halfInterval +
                    this.range(-quarterInterval, quarterInterval, useFloat)
            );
        }
        if (includeEnd) {
            set.push(end);
