...
Flyer.color = new (class FlyerColors {
    /**
    ...
    random(min, max) {
        ...
        var color = Flyer.random.pick(colors);

        min = min || 0;
        max = max || 255;

        return this.rgbToHex(
            color.r ? Flyer.random.range(min, max) : 0,
            color.g ? Flyer.random.range(min, max) : 0,
            color.b ? Flyer.random.range(min, max) : 0
        );
    }
    ...
    randomHSL(hMin, hMax, sMin, sMax, lMin, lMax) {
        var color = {
            h: Flyer.random.range(hMin, hMax),
            s: Flyer.random.range(sMin, sMax, true),
            l: Flyer.random.range(lMin, lMax, true)
        };
